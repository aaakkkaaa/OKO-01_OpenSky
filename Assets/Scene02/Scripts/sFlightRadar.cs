...
public class sFlightRadar : MonoBehaviour {
    ...
    */
    ...
    void Start () {
        ...
        myUnixStartTime = DateTimeOffset.Now.ToUnixTimeSeconds();
        myCurrentTime = 0;
        //myDeltaTime = 0;
    ...
    IEnumerator myFuncGetData()
    {
        ...
        while (true)
        {
            ...
            //MyLog("RawData", "@@@ myFuncGetData(): Время прихода ответа (myResponseTime) = " + myResponseTime + " myResponseUnixTime = " + myResponseUnixTime);
                ...
                for (int i = 0; i < myPlanesInZone; i++)
                {
                    ...
                    if (myPlanesHistory.TryGetValue(myKey, out myOnePlaneHist)) // история данного самолета
                    {
                        ...
                        else
                        {
                            ...
                            MyLog(myKey, "ProcData", "=== myFuncThread(): ключ = " + myKey + " Полученные данные. Raw Position = " + myPlane.RawPosition);
                            ...
                            MyLog(myKey, "ProcData", "=== myFuncThread(): О задержке полученных данных: myStartProcTime = " + myStartProcTime + " time_position = " + myOnePlanePars.time_position + " myPosDelay = " + myPosDelay);
                            ...
                            MyLog(myKey, "ProcData", "=== myFuncThread(): Будем искать плохие данные. Время: myStartProcTime = " + myStartProcTime);
                            MyLog(myKey, "ProcData", "=== myFuncThread(): Предсказано: myPredictedPosition = " + myPredictedPosition + " Получено: Raw Position = " + myPlane.RawPosition + " с поправкой на время: myResponsedPosition = " + myResponsedPosition);
                            MyLog(myKey, "ProcData", "=== myFuncThread(): myPredictedVector = " + myPredictedVector + " myResponsedVector = " + myResponsedVector + " myDeltaVector " + myDeltaVector);
                            MyLog(myKey, "ProcData", "=== myFuncThread(): Угол между направлением на прогноз и полученной точкой (град.) = " + myAngle);
                            if(k < 3)
                            {
                                myPlane.Position = myResponsedPosition;
                                myPlane.BadPosCounter = 0;
                                MyLog(myKey, "ProcData", "=== myFuncThread(): ключ = " + myKey + " В истории не больше 2х точек. Данные полета считаем достоверными, летим по ним. myPlane.Position = " + myPlane.Position);
                            }
                            ...
                            else if (myOnePlaneHist.BadPosCounter[k - 1] >= 3)
                            {
                                myPlane.Position = myResponsedPosition;
                                myPlane.BadPosCounter = 0;
                                MyLog(myKey, "ProcData", "=== myFuncThread(): ключ = " + myKey + " BadPos уже был раз подряд: " + myOnePlaneHist.BadPosCounter[k - 1] + ". Данные полета считаем достоверными, летим по ним. myPlane.Position = " + myPlane.Position);
                            }
                            else if (myDeltaVector.sqrMagnitude < 22500)
                            {
                                myPlane.Position = myResponsedPosition;
                                myPlane.BadPosCounter = 0;
                                MyLog(myKey, "ProcData", "=== myFuncThread(): ключ = " + myKey + " Отклонение от прогноза менее 150 метров. myDeltaVector.sqrMagnitude = " + myDeltaVector.sqrMagnitude + ". Данные полета считаем достоверными, летим по ним. myPlane.Position = " + myPlane.Position);
                            }
                            else if ((Mathf.Abs(myDeltaVector.y) > 200) && (myPredictedVector.y == 0.0f || (myPredictedVector.y != 0.0f && (Mathf.Abs(myResponsedVector.y / myPredictedVector.y)) > 5)))
                            {
                                ...
                                MyLog(myKey, "ProcData", "=== myFuncThread(): ключ = " + myKey + " Данные полета считаем недостоверными - " + myPlane.BadPosCounter + ". Скачок по вертикали (метров) = " + myDeltaVector.y + ", коэф. скорости = " + (myResponsedVector.y / myPredictedVector.y) + ". Летим по прогнозу. myPlane.Position = " + myPlane.Position);
                            }
                            else if (myPredictedHorVector.sqrMagnitude != 0 && (myResponsedHorVector.sqrMagnitude / myPredictedHorVector.sqrMagnitude) > 25.0f)
                            {
                                ...
                                myPlane.Position = myPredictedPosition;
                                myPlane.BadPosCounter = myOnePlaneHist.BadPosCounter[k - 1] + 1;
                                MyLog(myKey, "ProcData", "=== myFuncThread(): ключ = " + myKey + " Данные полета считаем недостоверными (скачок) - " + myPlane.BadPosCounter + ", летим по прогнозу. myPlane.Position = " + myPlane.Position);
                            }
                            else
                            {
                                myPlane.Position = myResponsedPosition;
                    ...
                    else
                    {
                        ...
                        // Распечатаем историю
                        MyLog(myKey, "Time\tPosTime\tPredict\tBad\tPosX\tPosY\tPosZ\tRawX\tRawY\tRawZ\tPitch\tYaw\tRoll\tSpeedX\tSpeedY\tSpeedZ", false);
        ...
        if (myPlaneVisValues != null)
        {
            ...
            foreach (String myKey in myKeys)
            {
                ...
                if (myPlane.GO)
                {
                    ...
                    float myDerivative = 0.0f; // Производная от функции расчета положения по Безье
                    float myDerivative2 = 0.0f;
                    ...
                    if (myLeftToTargetTime > 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (myPlane.NeedNewBezier)
                            {
                                ...
                                // Вычислим производные по нормированному времени в текущей точке для кривой, построенной по старым путевым точкам
                                float dX0 = myFuncDeriv_t(myPlane, myTim, 0);
                                float dZ0 = myFuncDeriv_t(myPlane, myTim, 2);
                                MyLog(myKey + "_Data", "Tim =\t" + myTim + "\tdX0 =\t" + dX0 + "\tdZ0 =\t" + dZ0 + "\tdX0/dZ0 =\t" + dX0/dZ0, false);
                                ...
                                MyLog(myKey + "_Data", "Frame\tTim\tPosX\tPosY\tPosZ\tYaw\tDelZ/DelX\tdZ/dX\tdZ/dX-2\tInter", false);
                            }
                            ...
                            myDerivative = myFuncDeriv_t(myPlane, myTim, 0) / myFuncDeriv_t(myPlane, myTim, 2);
                            myDerivative2 = myFuncDeriv_t2(myPlane, myTim, 0) / myFuncDeriv_t2(myPlane, myTim, 2);
                        ...
                        if (myPlaneHistCount >= 4) // если в истории больше четырех точек, то угол рыскания заменим, исходя из траектории по Безье
                        {
                            ...
                            MyLog(myKey + "_Data", myFrameCount + "\t" + (1.0f - (float)myLeftToTargetTime / myPlane.TrackTime) + "\t" + myPlane.GO.transform.position.x + "\t" + myPlane.GO.transform.position.y + "\t" + myPlane.GO.transform.position.z
                                + "\t" + myNewEu.y + "\t" + (myDeltaX / myDeltaZ) + "\t" + myDerivative + "\t" + myDerivative2 + "\t" + myInter, false);
                        }
                    }
    ...
    float myFuncDeriv_t(MyPlaneVisual myPlane, float myTim, int myIndex)
    {
        float myDeriv;
        myDeriv = (-(1.0f - myTim) * (1.0f - myTim) * myPlane.TrackPoints[0].position[myIndex]
    ...
    float myFuncDeriv_t2(MyPlaneVisual myPlane, float myTim, int myIndex)
    {
        float myDeriv;
        myDeriv = (-(1.0f - myTim) * (1.0f - myTim) * myPlane.TrackPoints[0].position[myIndex]
            + (1.0f - 4.0f * myTim + 3.0f * myTim * myTim) * myPlane.TrackPoints[4].position[myIndex]
            + (2.0f * myTim - 3.0f * myTim * myTim) * myPlane.TrackPoints[2].position[myIndex]
            + myTim * myTim * myPlane.TrackPoints[3].position[myIndex]) * 3.0f;
        return myDeriv;
    }
